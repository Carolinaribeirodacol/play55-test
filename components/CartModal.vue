<template>
  ...
  >
    <div class="bg-white rounded-lg w-full max-w-md p-6 shadow-lg relative">
      <header class="flex justify-between items-center border-b border-gray-200 w-full pb-2 mb-6">
      ...
      <ul class="divide-y divide-gray-200 max-h-96 overflow-y-auto mb-6">
        ...
        >
          ...
          <div class="flex-1 ml-4">
            ...
            <p class="font-semibold text-sm text-indigo-600 mt-1">{{ formatPrice(item.price) }}</p>
          </div>
      ...
      <div class="flex justify-between items-center font-semibold mb-6">
        <span>Total:</span>
        <span class="font-semibold text-indigo-600 text-lg">{{ formatPrice(cart.totalPrice) }}</span>
      </div>
